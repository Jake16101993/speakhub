...
async function applyDateDiscountsToOrder(orderData, sessionIds) {
  ...
  const sessionDateById = new Map(
    (sessions || []).map(s => [String(s.id), String(s.session_date || '')])
  );
  const dates = [...new Set(
    sessionIds.map(id => sessionDateById.get(String(id))).filter(Boolean)
  )];
  ...
  const discountByDate = new Map(
    (discounts || [])
      .map(x => [String(x.discount_date), Number(x.discount_percent || 0)])
  ...
  for (const sessionId of sessionIds) {
    const date = sessionDateById.get(String(sessionId));
    const pct = Number(discountByDate.get(String(date)) || 0);
    if (!(pct > 0)) continue;
