...
export default {
  async fetch(request) {
    ...
    try {
      ...
      try {
        const { data: attr } = await supabase.from('publisher_order_attributions').select('publisher_id,customer_id,visitor_id').eq('order_id', payment.order_id).maybeSingle();
        if (attr?.publisher_id) await supabase.from('publisher_events').insert({publisher_id:attr.publisher_id,visitor_id:attr.visitor_id,customer_id:attr.customer_id,order_id:payment.order_id,event_type:'PAID'});
      } catch (affiliateErr) { console.error('publisher paid event warning', affiliateErr); }
