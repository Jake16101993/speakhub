  ...
  async fetch(request) {
    ...
    try {
      ...
      try {
        ...
        if (attr?.publisher_id) await supabase.from('publisher_events').insert({publisher_id:attr.publisher_id,visitor_id:attr.visitor_id,customer_id:attr.customer_id,order_id:order.id,event_type:'PAID'});
      } catch (affiliateErr) { console.error('publisher reconcile event warning', affiliateErr); }
