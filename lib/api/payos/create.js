  ...
  async fetch(request) {
    ...
    try {
      ...
      if (
        ...
        !lastSeenMs ||
        Date.now() - lastSeenMs > 75 * 1000
      ) {
        return Response.json({ error: 'HOLD_INACTIVE' }, { status: 410 });
      }
      ...
      // The PayOS request is the slow external step. Once it returns,
      // save the payment and render the QR at the same time instead of serially.
      const [paymentInsert, qrImage] = await Promise.all([
      ...
      try {
        const { data: attr } = await supabase.from('publisher_order_attributions').select('publisher_id,customer_id,visitor_id').eq('order_id', order.id).maybeSingle();
        if (attr?.publisher_id) await supabase.from('publisher_events').insert({publisher_id:attr.publisher_id,visitor_id:attr.visitor_id,customer_id:attr.customer_id,order_id:order.id,event_type:'PAYMENT_QR'});
      } catch (affiliateErr) { console.error('publisher qr event warning', affiliateErr); }
