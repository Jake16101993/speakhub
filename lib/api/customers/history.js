  ...
  async fetch(request) {
    ...
    try {
      ...
      const { data: bookings, error } = await supabase
        ...
        .eq('status', 'CONFIRMED')
        // A paid + confirmed booking must remain visible in "My Schedule".
        // Do not additionally require orders.order_status = CONFIRMED:
        // manual/admin-created paid orders may use a different order_status.
      ...
      return new Response(JSON.stringify({ upcoming, completed }),{
        status:200,
        headers:{
